...
/*
load_convolutional_weights 的函式，用於從檔案中載入卷積層的權重。這個函式接受兩個參數：一個 layer 結構體 l 和一個檔案指標 fp。

首先，函式檢查 l 是否為二進位格式，如果是，則會跳過後續的處理。接著，函式檢查 l 的 numload 屬性，如果有設定，則將 l.n 設為 l.numload。接下來，計算需要讀取的權重數量，並使用 fread 函式從檔案中讀取偏置（biases）。

如果 l 啟用了批次正規化（batch normalization），且 dontloadscales 屬性未設定，則會從檔案中讀取縮放係數（scales）、滾動平均值（rolling mean）和滾動方差（rolling variance）。這些讀取的數據會在後續的條件判斷中進行一些調試輸出，但這些調試代碼目前被註解掉了。

接著，從檔案中讀取權重數據。如果 l 的 flipped 屬性為真，則會轉置權重矩陣。對於 GPU 的支援，如果 gpu_index 大於等於 0，則會將卷積層推送到 GPU。

在這段程式碼的後半部分，新增了一些代碼來將權重和偏置寫入到兩個新的檔案中。首先，開啟或創建兩個檔案 weights.bin 和 bias.bin。接著，分配記憶體來存儲權重、alpha 值和偏置。

如果啟用了批次正規化，則會計算 alpha 值和新的偏置值，否則 alpha 值設為 1，偏置值保持不變。然後，根據計算出的 alpha 值調整權重，並將調整後的權重和偏置寫入到對應的檔案中。最後，釋放分配的記憶體並關閉檔案。

這段程式碼的主要目的是從檔案中載入卷積層的權重和偏置，並根據需要進行一些轉換和調整，然後將結果寫入新的檔案中。
...
/*
load_weights_upto 的函式，用於從指定的檔案中載入神經網路的權重。這個函式接受四個參數：一個指向 network 結構的指標 net，一個字串 filename，以及兩個整數 start 和 cutoff。

首先，如果定義了 GPU，且網路的 gpu_index 大於等於 0，則會設定 CUDA 裝置。接著，函式會嘗試以二進位模式開啟指定的檔案。如果檔案無法開啟，則會呼叫 file_error 函式來處理錯誤。

接下來，程式碼從檔案中讀取三個整數，分別是 major、minor 和 revision，這些數值代表了權重檔案的版本。如果版本號符合特定條件，則會從檔案中讀取一個 size_t 型別的數值並存入 net->seen。否則，會讀取一個整數並將其轉換為 size_t 型別後存入 net->seen。

接著，程式碼會根據版本號來決定是否需要轉置權重。然後，進入一個迴圈，從 start 開始遍歷網路中的每一層，直到 cutoff 或網路的層數 net->n。在迴圈中，程式碼會根據每一層的類型來載入相應的權重。例如，對於卷積層和反卷積層，會呼叫 load_convolutional_weights 函式；對於全連接層，會呼叫 load_connected_weights 函式，並根據是否需要轉置來決定參數。

此外，程式碼還處理了其他類型的層，如批次正規化層、CRNN 層、RNN 層、LSTM 層和 GRU 層，並根據層的類型呼叫相應的函式來載入權重。對於本地層，程式碼會讀取偏差和權重，並在 GPU 上推送本地層（如果定義了 GPU）。

最後，函式會在標準錯誤輸出中打印 "Done!"，並關閉檔案。這段程式碼的主要目的是根據不同層的類型和版本號，從檔案中載入相應的權重到神經網路中。
*/
